...
use crate::{model::AppContext, AppError, Config};
...
fn get_fallback_file(config: &Config) -> Result<PathBuf, AppError> {
...
pub fn create_file_service(app_ctx: Arc<AppContext>) -> Result<ServeDir<ServeFile>, AppError> {
    let fallback_file_path = get_fallback_file(&app_ctx.config)?;
    Ok(
        ServeDir::new(&app_ctx.config.website.web_root)
